...
describe("POST /login", () => {
    ...
    it("should login a user and return a JWT token", (done) => {
        chai.request.execute(app) // Initiating a request to the app
            .post("/login") // Sending a POST request to the /login endpoint
            .send({ email: "mukhammadaliweb@gmail.com", password: "qwerty" }) // Sending login credentials
            .end((err, res) => {
                if (err) {
                    done(err);
                    return;
    ...
    it("should return 400 for invalid credentials", (done) => {
        chai.request.execute(app) // Initiating a request to the app
            .post("/login") // Sending a POST request to the /login endpoint
            .send({ email: "mukhammadaliweb@gmail.com", password: "wrong password" }) // Sending invalid login credentials
            .end((err, res) => {
                if (err) {
                    done(err);
                    return;
